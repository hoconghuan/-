<template>
  <div class="register">
    ...
    <div class="deparment">
      <div class="leftNav">
        <ul>
          <li @click="changeIndex(index)" v-for="(deparment, index) in hospitalStore.hospitalDepartmentArr"
      ...
      <div class="deparmentInfo">
        <!-- 用一个div代表:大科室与小科室 -->
        <div class="showDeparment" v-for="deparment in hospitalStore.hospitalDepartmentArr" :key="deparment.depcode">
          <h1 class="cur">{{ deparment.depname }}</h1>
          <!-- 每一个大的科室下小科室 -->
          <ul>
            <li v-for="item in deparment.children" :key="item.depcode" @click="login">
              {{ item.depname }}
            </li>
          </ul>
        </div>
