<template>
  <div class="wrap">
    <!-- 顶部结构 -->
    <div class="top">
      <div class="hosname">{{ responseData.baseMap?.hosname }}</div>
      <div class="line"></div>
      <div>{{ responseData.baseMap?.bigname }}</div>
      <div class="dot">.</div>
      <div class="hosdeparment">{{ responseData.baseMap?.depname }}</div>
    </div>
    <!-- 中间展示日期的结构 -->
    <div class="center">
      <h1 class="time">{{ responseData.baseMap?.workDateString }}</h1>
      <div class="container">
        <div
          class="item"
...
const fetchData = async () => {
  ...
  if (result.code === 200) {
    ...
    workTime.value = responseData.value.bookingScheduleList[0];
